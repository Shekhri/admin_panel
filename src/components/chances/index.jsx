...
const Chances = () => {
  ...
  const handleDelete = (id) => {
    axios.delete(`https://apiastro1.vtormetallmm.ru/possibilities/${id}`)
      .then(() => {
  ...
  const handleSubmitAdd = (e) => {
    ...
    axios.post('https://apiastro1.vtormetallmm.ru/possibilities', body)
      .then(res => {
  ...
  const handleSubmitEdit = () => {
    if (taskToEdit && taskToEdit.id && titleRef.current.value && descRef.current.value && priceRef.current.value) {
      ...
      axios.put(`https://apiastro1.vtormetallmm.ru/possibilities/${taskToEdit.id}`, body)
        .then(res => {
          ...
          <TableBody>
            {tasks.map(task => (
              <TableRow className={Styles.table} key={task.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                ...
                <TableCell align="right">{task.title.slice(0,10)}...</TableCell>
                <TableCell align="right">{task.desc.slice(0, 10)}...</TableCell>
