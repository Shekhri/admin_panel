...
const News = () => {
  ...
  const handleDelete = (id) => {
    axios.delete(`https://apiastro1.vtormetallmm.ru/news/1/${id}`)
      .then(() => {
  ...
  const handleSubmitAdd = (e) => {
    ...
    axios.post('https://apiastro1.vtormetallmm.ru/news', body)
      .then(res => {
  ...
  const handleSubmitEdit = () => {
    if (taskToEdit && taskToEdit.id && titleRef.current.value && imgref.current.value && descRef.current.value) {
      const body = {
        title: titleRef.current.value,
        desc: descRef.current.value,
        image: imgref.current.value,
      };
      axios.put(`https://apiastro1.vtormetallmm.ru/news/1/${taskToEdit.id}`, body)
        .then(res => {
      ...
      <ModalCommon open={openAddModal} handleClose={handleCloseAddModal}>
        <div className={Styles.card__modal}>
          ...
          <form className={Styles.form} onSubmit={handleSubmitAdd}>
            ...
            <input className={Styles.input__top} ref={imgref} type='url' placeholder='image' />
      ...
      <ModalCommon open={openEditModal} handleClose={handleCloseEditModal}>
        <div className={Styles.card__modal}>
          ...
          <form className={Styles.form}  onSubmit={handleSubmitEdit}>
            ...
            <input className={Styles.input__top} ref={imgref} type='url' placeholder='image' defaultValue={taskToEdit?.image} />
            <button className={Styles.modal__button}>Save</button>
          </form>
        </div>
      </ModalCommon>
    </div>
